...
_vnckey = [ 23,82,107,6,35,78,88,7 ]

# This is a departure from the original code.
#bytebit = [ 0200, 0100, 040, 020, 010, 04, 02, 01 ] # original
_bytebit = [ 0o1, 0o2, 0o4, 0o10, 0o20, 0o40, 0o100, 0o200 ] # VNC version

# two password functions for VNC protocol.
def decrypt_passwd(data):
  '''VNC protocol decrypt passwd'''
  dk = _deskey(pack('8B', *_vnckey), True)
  return _desfunc(data, dk)

def generate_response(passwd, challange):
  '''VNC protocol generate response'''
  ek = _deskey((passwd.encode()+b'\x00'*8)[:8], False)
  return _desfunc(challange[:8], ek) + _desfunc(challange[8:], ek)
...
_totrot = [ 1,2,4,6,8,10,12,14,15,17,19,21,23,25,27,28 ]
...
def _deskey(key, decrypt):      # Thanks to James Gillogly & Phil Karn!
    key = unpack('8B', key)
...
def encrypt(clear):
  ...
  >>> from mypielib.d3des import encrypt as vncrypt
  >>> vncrypt('zahT9aib')
  '7MjSEypOX1U='
  >>> vncrypt('Quiali1i')
  '3hxs4eyguyQ='
  >>> vncrypt('thieNga1Yahh9oaY')
  'jigZ/CZp6As='

  ```
  '''
  clear = (bytes(clear,encoding='raw_unicode_escape') + b'\x00' * 8)[:8]
  strkey = b''.join([bytes(chr(x),encoding='raw_unicode_escape') for x in _vnckey])
  key = _deskey(strkey,False)
  crypted = _desfunc(clear, key)
